"""
Fast Dataset Analysis for LLM Fine-Tuning (with sampling for large datasets)
Analyzes JSONL ChatML format data and generates a detailed report quickly
"""
...
class FastDatasetAnalyzer:
    def __init__(self, data_dir="splits_full", train_sample_size=50000):
    ...
    def load_jsonl_sample(self, file_path, sample_size=None):
        """Load JSONL file with optional sampling for large files."""
        total_lines = self.count_lines(file_path)

        if sample_size and total_lines > sample_size:
            # Sample random lines
        ...
        return data, total_lines, is_sampled

    def estimate_tokens(self, text):
        """Rough token estimation (1 token ≈ 4 characters for English)."""
        return len(text) // 4

    def analyze_split(self, data, split_name, total_count=None, is_sampled=False):
        """Analyze a single data split."""
        print(f"Analyzing {split_name} split...")
        if is_sampled:
            print(f"  Using sample of {len(data):,} from {total_count:,} total examples")

        stats = {
            'num_examples': total_count if total_count else len(data),
            'num_analyzed': len(data),
        ...
        # Scale up token estimate if sampled
        if is_sampled and total_count:
            stats['total_tokens_estimated_full'] = int(stats['avg_tokens_per_example'] * total_count)
        else:
            stats['total_tokens_estimated_full'] = stats['total_tokens_estimated']
    ...
    def analyze_all_splits(self):
        ...
        print("Starting fast analysis with sampling...\n")

        # Train set with sampling
        if self.train_path.exists():
            data, total, is_sampled = self.load_jsonl_sample(
                self.train_path,
                sample_size=self.train_sample_size
            )
            self.stats['train'] = self.analyze_split(data, 'train', total, is_sampled)
        else:
            print(f"Warning: {self.train_path} not found")

        # Valid and test sets (small enough to load fully)
        for split_name, path in [('valid', self.valid_path), ('test', self.test_path)]:
            if path.exists():
                data, total, is_sampled = self.load_jsonl_sample(path, sample_size=None)
                self.stats[split_name] = self.analyze_split(data, split_name, total, is_sampled)
            else:
        ...
        self.stats['overall']['total_tokens_estimated'] = sum(
            self.stats[split].get('total_tokens_estimated_full', 0)
            for split in ['train', 'valid', 'test']
        )
    ...
    def generate_report(self, output_file="dataset_analysis.txt"):
        ...
        with open(output_file, 'w', encoding='utf-8') as f:
            ...
            # Note about sampling
            if self.stats['train'].get('is_sampled'):
                f.write(f"NOTE: Training set analyzed using {self.train_sample_size:,} sample examples\n")
                f.write(f"      (Statistics are projected from sample to full dataset)\n")
            ...
            for split in ['train', 'valid', 'test']:
                ...
                sample_note = f" (analyzed {self.stats[split].get('num_analyzed', 0):,})" if self.stats[split].get('is_sampled') else ""
                f.write(f"  {split.capitalize():8s}: {num:10,} examples ({pct:5.2f}%){sample_note}\n")
            f.write("\n")
            ...
            for split in ['train', 'valid', 'test']:
                ...
                f.write(f"  Total Tokens (estimated): ~{stats.get('total_tokens_estimated_full', 0):,}\n")
            ...
            f.write("Card Types Distribution (Training Set Sample):\n")
            ...
            f.write("Card Rarities Distribution (Training Set Sample):\n")
            ...
            if min_response_len > 0:
                f.write(f"  ✓ No empty responses detected in sample\n")
            ...
            if num_train > 1_000_000:
                ...
                f.write(f"    - LoRA/QLoRA for efficiency (or full fine-tuning with DeepSpeed ZeRO-3)\n")
                ...
                f.write(f"    - Multi-GPU training highly recommended\n")
            ...
            data, _, _ = self.load_jsonl_sample(self.train_path, sample_size=3)
            ...
            for i, example in enumerate(data, 1):
                ...
                for msg in example.get('messages', []):
                    ...
                    content = msg.get('content', '')
                    content_display = content if len(content) <= 200 else content[:197] + "..."
                    f.write(f"  [{role}]: {content_display}\n")
                f.write("\n")
...
def main():
    ...
    print("Starting FAST dataset analysis with sampling...")
    print("=" * 80)

    analyzer = FastDatasetAnalyzer(data_dir="splits_full", train_sample_size=50000)
